...
import { DIA_MADRES_FILTER_KEY, ENABLE_MOTHERS_DAY_CATALOG } from '../constants/promoProducts'
import {
...
describe('mothersDayCatalog', () => {
  if (ENABLE_MOTHERS_DAY_CATALOG) {
    ...
    it('permanece desactivado sin borrar la logica para reactivarla despues', () => {
      expect(isMothersDayCatalogSeason('2026-05-01')).toBe(false)
      expect(isMothersDayCatalogLocked('2026-05-08')).toBe(false)
      expect(resolveInitialFlowerType('2026-05-03')).toBe('all')
      expect(isMothersDayProductId('DM_Tulipanes')).toBe(false)
      expect(cartSupportsLockedMothersDayDates([{ id: 'Bouquet_rosas' }])).toBe(true)
      expect(isLockedMothersDayDateBlockedForCart('2026-05-08', [{ id: 'Bouquet_rosas' }])).toBe(false)
      expect(allowsMothersDaySundayDelivery('2026-05-10')).toBe(false)
      expect(isSundayDeliveryBlocked('2026-05-10')).toBe(true)
      expect(DIA_MADRES_FILTER_KEY).toBe('dia_madres')
    })
  }
})
